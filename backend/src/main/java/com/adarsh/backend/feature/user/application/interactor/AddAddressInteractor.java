...
public class AddAddressInteractor implements AddAddressUseCase {
    ...
    public AddAddressResult execute(String email, AddAddressCommand command) {
        com.adarsh.backend.feature.user.domain.model.User user = userCommandRepository.findByEmail(email).orElseThrow(() -> new UserNotFoundException("User not found with email: " + email));
