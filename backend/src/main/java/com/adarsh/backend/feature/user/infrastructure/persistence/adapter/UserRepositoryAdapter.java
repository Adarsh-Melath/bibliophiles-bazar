...
public class UserRepositoryAdapter implements UserCommandRepository, UserQueryRepository {
    ...
    public Optional<User> findById(Long id) {
        return userJpaRepository.findByIdAndDeletedFalse(id).map(mapper::toDomain);
    }

    @Override
    public Optional<User> findByEmail(String email) {
        return userJpaRepository.findByEmailAndDeletedFalse(email).map(mapper::toDomain);
    }
    ...
    public PageResult<User> search(UserSearchCriteria criteria, PageQuery query) {
        ...
        Page<UserEntity> springPage = userJpaRepository.searchUsersAndDeletedFalse(criteria.searchKeyword(), criteria.role(), criteria.enabled(), pageable);
